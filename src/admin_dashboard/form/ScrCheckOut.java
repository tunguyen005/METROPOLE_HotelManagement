...
public class ScrCheckOut extends JPanel {
    ...
    private void initComponents() {
        // Tiêu đề
        JLabel jLabel1 = new JLabel("Dashboard / CHECK-OUT");
        ...
        btnCheckOut.setFont(new Font("Calistoga", Font.PLAIN, 12));
        btnCheckOut.setIcon(IconFontSwing.buildIcon(GoogleMaterialDesignIcons.CHECK_CIRCLE, 35, Color.BLACK));
    ...
    private void performCheckOut() {
        ...
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn một đặt phòng để check-out!", "Lỗi", JOptionPane.WARNING_MESSAGE);
            return;
        }

        int selectedId = (int) tableModel.getValueAt(selectedRow, 0);
        try {
            String sql = "UPDATE booking SET status = 'CHECKED_OUT' WHERE id = ? AND status = 'CHECKED_IN'";
            int rowsAffected = dbConn.executeUpdate(sql, selectedId);

            if (rowsAffected > 0) {
                JOptionPane.showMessageDialog(this, "Check-out thành công!", "Thành công", JOptionPane.INFORMATION_MESSAGE);
                loadTableData();
            } else {
                JOptionPane.showMessageDialog(this, "Không thể check-out. Đặt phòng có thể đã được cập nhật trạng thái!", "Lỗi", JOptionPane.ERROR_MESSAGE);
            }
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(this, "Lỗi khi thực hiện check-out: " + e.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void exportReport() {
        ...
        try {
            ...
            InputStream reportStream = getClass().getResourceAsStream("/reports/CheckOutReport.jrxml");
            if (reportStream == null) {
                throw new IOException("Không tìm thấy file CheckOutReport.jrxml trong resources");
            }
            ...
            if (choice == 0) { // PDF
                ...
                fileChooser.setSelectedFile(new java.io.File("CheckOut_Report.pdf"));
                ...
                JFileChooser fileChooser = new JFileChooser();
                fileChooser.setSelectedFile(new java.io.File("CheckOut_Report.xlsx"));
                if (fileChooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
                    FileOutputStream fos = new FileOutputStream(fileChooser.getSelectedFile());
                    try {
